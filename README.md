# Incompatible with @google-cloud/storage?

`npm start` shows the errors, which goes away when commenting out the `Storage` module on `src/index.js, line 5:`

```js
const {Storage} = require('@google-cloud/storage');
```

Compared to the default starter kit, I've added a few more `fallback` options in <./webpack.config.js>.
